...
| 首屏体积预算 | **严守 ≤ 100 KB gzip**；若 WASM 拆分后首屏超预算，**放弃 Phase 2** 并汇报 |
| 现有 `converter.ts` 处理 | **直接删除**（无外部引用），`encoder.ts` 整体替换 |
| 默认编码模式 | **WASM 真无损默认选中**；首次触发通过 idle prefetch（`requestIdleCallback`）在页面空闲时后台加载，避免用户第一次拖图要干等 |
