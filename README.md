...
- **序列图 → animated WebP**（Phase 5）：
  ...
  - 复用视频参数的 FPS / 质量 / 循环次数
- **序列合成抠色**（Phase 6）：
  - 合成按钮旁 checkbox "扣掉左上角背景色"（默认关）
  - 以**第一帧 (0,0)** 像素 RGB 作为 key color
  - 每像素 Chebyshev ±10 容差（PNG 精确匹配，JPEG 容忍少量压缩抖动）
  - 匹配像素的 alpha 置 0，输出 WebP 背景透明
  - 状态持久化到 `localStorage`
...
生产构建首屏 **93.30 KB gzip**（HTML + CSS + JS 一次性加载）。各编码器 chunk 只在进入对应模式时才加载。静态图编码 Worker 是独立 chunk，运行在后台线程。序列合成（含抠色）复用 Phase 3 的 ffmpeg，零新依赖。
...
- ✅ Phase 6: 序列合成 chroma-key 抠色（左上角像素为背景色）
- ⏳ Phase 7: 多文件并发（worker pool） / 视频 trim / 多线程 ffmpeg / 容差滑块
- ⏳ Phase 8: Tauri 桌面版（可输出到原目录）
- ⏳ Phase 9: AVIF 输出支持（可选）
